...
exports.SignUp = async (req, res) => {
  ...
  if (dup_email) {
    res.send(res.__("user").duplicate_entry);
    ...
    res.send(res.__("user").user_created);
...
exports.LogIn = async (req, res, next) => {
  ...
  (email = req.body.email),
    (password = req.body.password),
    (loggin_user = await User.findOne({
  ...
  if (loggin_user) {
    ...
    return res.status(400).json({
      success: false,
      message: "Invalid Password or Email or User Does not exists ",
    });
  }
};
  ...
  try {
    ...
    if (there) {
      ...
      if (rating <= 5) {
        ...
        const z = rate.rating;
        if (z == "0") {
          const find1 = await Barber.update(
            { rating: rating },
            { where: { id: barber_id } }
          );
          if (find1) {
        ...
        const find = await Barber.update(
          { rating: Updaterate },
          { where: { id: barber_id } }
        );
        if (find) {
        ...
        return res
          .status(400)
          .json({ success: false, message: "Rate between 1 -  5" });
      }
    }
  } catch (error) {
    console.log(error);
    return res
      .status(500)
      .json({ success: false, message: "Barber is not registred" });
  }
};
  ...
  try {
    ...
    if (there) {
      ...
      if (rating <= 5) {
        ...
        const z = rate.rating;
        if (z == "0") {
          const find1 = await Saloon.update(
            { rating: rating },
            { where: { id: saloon_id } }
          );
          if (find1) {
        ...
        const Updaterate = (rating + z) / 2;
        const find = await Saloon.update(
          { rating: Updaterate },
          { where: { id: saloon_id } }
        );
        if (find) {
    ...
    return res
      .status(500)
      .json({ success: false, message: "Saloon is not registred" });
  }
};
